...
class lecture_details
{
    public function verifyUser($dbo,$un,$pw)
    {
        $rv=["id"=>-1,"status"=>"ERROR"];
          $c="select id,password from lecture_details where user_name=:un";
    ...
    public function getCoursesInASession($dbo,$sessionid,$facid)
    {
      ...
      course_assignment as ca,course_details as cd
    ...
    {
      $name='';
      $c="select name from lecture_details where id=:id";
          $s=$dbo->conn->prepare($c);
          try{
             $s->execute([":id"=>$facid]);
             if($s->rowCount()>0)
